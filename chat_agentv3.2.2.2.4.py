"""
chat_agentv3.2.2.3.py — AI agent that talks to Amazon Bedrock (Converse API) and
can drive a real Chrome browser (via PLAYWRIGHT) to join and participate in
Microsoft Teams meetings. This is the Bedrock port of the chat_agentv3 line
(previously Ollama-backed).

v3.2.2.3 change vs 3.2.2: adds a human pager. When a meeting participant asks to
speak to a real person / escalate / get David, the agent pages the on-call
engineer(s) over SSH via the pager script (page_human tool + a hard trigger in
--converse). A --page CLI flag tests the pager without joining a meeting.
...
API and lets it call local Python "tools":
    ...
    send_teams_message, leave_meeting, get_patch_status, page_human.
...
--------------------------------------------------------------------------------
    ...
    python chat_agentv3.2.2.3.py --setup-teams

--------------------------------------------------------------------------------
USAGE
--------------------------------------------------------------------------------
    # Plain chat (one-shot):
    python chat_agentv3.2.2.3.py "Explain what an AI agent is in one sentence."

    # Interactive chat loop (tool-enabled, remembers context):
    python chat_agentv3.2.2.3.py

    # One-shot tool call:
    python chat_agentv3.2.2.3.py --tools "What time is it in Singapore?"

    # Config comes from .env (see .env.example). Example keys:
    #   NLB_SR_ACCOUNT, NLB_CASE_WINDOWS, NLB_CASE_RHEL, SSH_HOST, MONITOR_SCRIPT,
    #   PAGER_SCRIPT, PAGER_TO, PAGER_REGION, SLACK_WEBHOOK_URL
    python chat_agentv3.2.2.3.py --tools "what's the patch status?"


    # Autonomous Teams mode. If a Teams meeting tab is already open, REUSE it:
    python chat_agentv3.2.2.3.py --converse

    # Or pass a link to join a meeting that isn't open yet:
    python chat_agentv3.2.2.3.py --converse https://teams.microsoft.com/meet/<id>?p=<key>

    # Test the human pager from the CLI (no meeting needed). Pages the on-call
    # engineer via PAGER_SCRIPT; --to/--case/--details are optional overrides:
    python chat_agentv3.2.2.3.py --page
    python chat_agentv3.2.2.3.py --page --to kmarawa,mbalajr
    python chat_agentv3.2.2.3.py --page --case 178... --details "2 failed: i-abc"
...
#   ssh <SSH_HOST> "<PAGER_SCRIPT> <caseId> --to <alias,...> \
#                   [--region <region>] [--details '<text>']"
# Valid aliases are fixed by the pager script; we validate against them so a
# typo/hallucinated alias can never reach the script.
PAGER_SCRIPT = os.environ.get(
    "PAGER_SCRIPT", "/home/davidakc/nlb_patch_pagerv1.2.sh"
)
# Default recipients (comma-separated aliases) when the user doesn't name one.
PAGER_TO = os.environ.get("PAGER_TO", "davidakc")
# Region passed to the pager (its own default is ap-southeast-1).
PAGER_REGION = os.environ.get("PAGER_REGION", "ap-southeast-1")
# Case to page with when the request doesn't name one. Falls back to the
# Windows case, then the legacy NLB_CASE.
PAGER_CASE = os.environ.get("PAGER_CASE", "")
# Aliases the pager script accepts. Anything not in this set is rejected.
PAGER_VALID_ALIASES = {"davidakc", "kmarawa", "mbalajr"}
...
def _default_pager_case() -> str:
    """Case id to page with when the request doesn't name one.

    Priority: explicit PAGER_CASE -> first configured patch target's case
    (usually Windows) -> legacy NLB_CASE.
    """
    if PAGER_CASE and str(PAGER_CASE).isdigit():
        return str(PAGER_CASE)
    for t in PATCH_TARGETS:
        if t.get("case") and str(t["case"]).isdigit():
            return str(t["case"])
    return str(NLB_CASE) if str(NLB_CASE).isdigit() else ""


def _clean_aliases(to: str) -> list:
    """Return the valid aliases from a comma/space separated string.

    Silently drops anything not in PAGER_VALID_ALIASES so a hallucinated or
    mistyped alias can never be passed to the pager script."""
    raw = [a.strip().lower() for a in re.split(r"[,\s]+", to or "") if a.strip()]
    return [a for a in raw if a in PAGER_VALID_ALIASES]


def page_human(case: str = "", to: str = "", details: str = "") -> str:
    """Page the on-call engineer(s) via the NLB pager script over SSH.

    Use this when a meeting participant asks to speak to a real person, wants
    to escalate, or asks for David/a human to step in.

    Runs: ssh <SSH_HOST> "<PAGER_SCRIPT> <case> --to <aliases>
                          --region <region> [--details '<text>']"

    `case`    - SR case id; defaults to the current session's case.
    `to`      - comma-separated aliases (davidakc, kmarawa, mbalajr); defaults
                to PAGER_TO. Invalid aliases are dropped.
    `details` - optional free text / failed-instance lines.
    Returns a short human-readable result string.
    """
    case = str(case).strip() if case else ""
    if not (case and case.isdigit()):
        case = _default_pager_case()
    if not case:
        log.error("[pager] no case id available to page with")
        return "ERROR: No case id configured to page with."

    aliases = _clean_aliases(to) or _clean_aliases(PAGER_TO)
    if not aliases:
        log.error("[pager] no valid aliases (got to=%r, default=%r)", to, PAGER_TO)
        return ("ERROR: No valid pager recipient. Valid aliases: "
                + ", ".join(sorted(PAGER_VALID_ALIASES)) + ".")

    # Build the remote command. Quote details with shlex so it survives the
    # remote shell as a single argument (and can't inject extra commands).
    import shlex
    remote_parts = [PAGER_SCRIPT, case, "--to", ",".join(aliases),
                    "--region", PAGER_REGION]
    if details.strip():
        remote_parts += ["--details", details.strip()]
    remote_cmd = " ".join(shlex.quote(p) for p in remote_parts)

    log.info("[pager] paging %s for case %s: ssh %s %r",
             aliases, case, SSH_HOST, remote_cmd)
    try:
        proc = subprocess.run(
            ["ssh", SSH_HOST, remote_cmd],
        ...
        log.error("[pager] ssh not found on PATH")
        return "ERROR: ssh not found on PATH. Install/enable OpenSSH client."
    except subprocess.TimeoutExpired:
        log.error("[pager] timed out after 120s (case %s)", case)
        return "ERROR: Pager timed out (no result within 2 minutes)."

    out = ((proc.stdout or "") + (proc.stderr or "")).strip()
    log.info("[pager] case %s exit=%s raw output:\n%s", case, proc.returncode, out)

    if proc.returncode != 0:
        tail = "\n".join(out.splitlines()[-10:]) if out else "(no output)"
        log.error("[pager] non-zero exit %s (case %s)", proc.returncode, case)
        return f"ERROR: Pager exited {proc.returncode}.\n{tail}"

    who = ", ".join(aliases)
    return f"Paged {who} for case {case}."
...
# Phrases that mean "get me a real person / escalate / page the on-call".
_HUMAN_REQUEST_RE = re.compile(
    r"\b("
    r"real person|speak (?:to|with) (?:a )?(?:human|someone|person)|"
    r"talk (?:to|with) (?:a )?(?:human|someone|person)|"
    r"get (?:me )?(?:a )?(?:human|someone|person)|"
    r"human (?:please|help|being)?|escalate|page (?:the )?(?:on[- ]?call|human|someone|david|engineer)|"
    r"call (?:david|someone|a human|the engineer)|"
    r"need (?:a |an )?(?:human|real person|someone real|engineer)|"
    r"can (?:someone|a human|david) (?:join|help)|is anyone (?:there|real)"
    r")\b",
    re.IGNORECASE,
)
...
def _is_human_request(text: str) -> bool:
    """True when the message asks to reach a real person / escalate / page."""
...
def converse_teams(poll_seconds: int = 3, max_chat_fails: int = 3) -> None:
    ...
    try:
        while True:
            ...
            # HARD TRIGGER: asked for a real person / escalation -> page the
            # on-call engineer, then acknowledge in chat. Checked BEFORE the
            # status trigger so "can a human help?" isn't mistaken for a status
            # question.
            if _is_human_request(latest):
                print("  [converse] human requested -> paging on-call")
                # Concise, human-readable escalation reason for the page. Names
                # the requester and quotes their message so the on-call knows
                # who asked and why, without a long transcript.
                reason = f"Human escalation requested in Teams by {author}: {latest}"
                result = page_human(details=reason)
                if result.startswith("ERROR:"):
                    ack = ("I tried to page a human but hit an issue - I'll "
                           "alert the team another way.")
                    notify_slack(f":warning: Pager failed from Teams: {result}")
                else:
                    ack = ("I've paged the on-call engineer - someone will "
                           "join as soon as they can.")
                send_teams_message(ack)
...
TOOLS_SCHEMA = [
    ...
    },
    {
        ...
        "function": {
            "name": "page_human",
            "description": (
                "Page the on-call engineer(s) when a meeting participant asks "
                "to speak to a real person, wants to escalate, or asks for a "
                "human/David to step in. Sends a page over SSH via the pager "
                "script. After paging, tell the person someone will join."
            ),
            "parameters": {
                "type": "object",
                "properties": {
                    "to": {"type": "string",
                           "description": "Optional comma-separated aliases to page (davidakc, kmarawa, mbalajr). Leave blank for the default recipient."},
                    "case": {"type": "string",
                             "description": "Optional SR case id. Leave blank to use the current session's case."},
                    "details": {"type": "string",
                                "description": "Optional short reason or failed-instance lines to include in the page."},
                },
                "required": [],
            },
        },
...
def main() -> None:
    ...
    # Test the human pager from the CLI (no meeting needed):
    #   python chat_agentv3.2.2.3.py --page
    #   python chat_agentv3.2.2.3.py --page --to kmarawa,mbalajr
    #   python chat_agentv3.2.2.3.py --page --case 178... --details "2 failed: i-abc"
    if args and args[0] == "--page":
        ...
        while i < len(rest):
            ...
            if a == "--to" and i + 1 < len(rest):
                to = rest[i + 1]; i += 2
            elif a == "--case" and i + 1 < len(rest):
                case = rest[i + 1]; i += 2
            elif a == "--details" and i + 1 < len(rest):
                details = rest[i + 1]; i += 2
            else:
                print(f"[page] ignoring unrecognized arg: {a!r}")
                i += 1
        if not details:
            details = "Manual pager test from the CLI (--page)."
        print(f"[page] paging (to={to or PAGER_TO!r}, case={case or '<default>'!r})...")
        result = page_human(case=case, to=to, details=details)
        print(result)
    ...
    if args and args[0] == "--converse":
        ...
        if already_in_meeting:
            ...
            print("ERROR: No Teams meeting is open and no link was provided.\n"
                  ...
                  "  python chat_agentv3.2.2.3.py --converse <teams-meeting-url>",
                  file=sys.stderr)
            sys.exit(1)
