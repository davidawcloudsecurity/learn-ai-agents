"""
chat_agentv3.2.1.py — AI agent that talks to Amazon Bedrock (Converse API) and can
...
--------------------------------------------------------------------------------
    ...
    python chat_agentv3.2.1.py --setup-teams

--------------------------------------------------------------------------------
USAGE
--------------------------------------------------------------------------------
    # Plain chat (one-shot):
    python chat_agentv3.2.1.py "Explain what an AI agent is in one sentence."

    # Interactive chat loop (tool-enabled, remembers context):
    python chat_agentv3.2.1.py

    # One-shot tool call:
    python chat_agentv3.2.1.py --tools "What time is it in Singapore?"

    # Config comes from .env (see .env.example). Example keys:
    #   NLB_CASE, NLB_SR_ACCOUNT, SSH_HOST, MONITOR_SCRIPT, SLACK_WEBHOOK_URL
    python chat_agentv3.2.1.py --tools "what's the patch status?"


    # Autonomous Teams mode: join the meeting, then watch the chat and reply
    # on David's behalf until someone says 'bye' (no timeout):
    python chat_agentv3.2.1.py --converse https://teams.microsoft.com/meet/<id>?p=<key>
